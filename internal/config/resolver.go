...
// Resolver implements platform.ScopedResolver by loading TOML config
...
func NewResolver(store platform.ConfigStore, botName string) *Resolver {
	return &Resolver{
		store:   store,
		botName: botName,
	}
...
func (r *Resolver) loadViper(scope platform.ConfigScope) (*viper.Viper, error) {
...
func (r *Resolver) ResolveBot(featureKey string, target any) error {
	scope := platform.ConfigScope{Bot: r.botName, GuildID: 0}
...
func (r *Resolver) ResolveGuild(featureKey string, guildID snowflake.ID, target any) error {
	...
	botScope := platform.ConfigScope{Bot: r.botName, GuildID: 0}
	...
	guildScope := platform.ConfigScope{Bot: r.botName, GuildID: guildID}
...
// `any` to satisfy the platform.ScopedResolver interface; callers
// should use platform.ResolveAll[T] for typed access.
...
func (r *Resolver) Invalidate(_ platform.ConfigScope) {}
