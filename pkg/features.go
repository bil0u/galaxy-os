...
var (
	knowFeatures = map[string]reflect.Type{}
	mu           sync.Mutex
)
...
type Feature interface {
	...
	IsEnabled() bool
	// IsProperlyConfigured returns an error if the feature is not configured properly
	IsProperlyConfigured() error
...
func RegisterFeature[T Feature](name string) {
	...
	defer mu.Unlock()
	var zero T
	knowFeatures[name] = reflect.TypeOf(zero)
}

type TomlFeaturesDefinition struct {
	RawData map[string]any `toml:"features"`
}

// If the bot name is empty, it means features are defined using [features.<feature_name>] instead of [features.<bot_name>.<feature_name>]
func (fd TomlFeaturesDefinition) GetFeaturesDefinitions(botName string) map[string]any {

	if botName == "" {
		return fd.RawData
	}
	botFeaturesDef, ok := fd.RawData[botName].(map[string]any)
	if !ok && botName != "" {
		slog.Error(fmt.Sprintf("bot '%s' not found in features definition", botName))
		return nil
	}
	return botFeaturesDef

}

// Custom unmarshalling logic for GuildFeatureSet
// If the botName is empty, features will be decoded from [features.<feature_name>] instead of [features.<bot_name>.<feature_name>]
func (fd TomlFeaturesDefinition) ToFeatures(botName string, superset []Feature) []Feature {

	features := []Feature{}

	featuresDefs := fd.GetFeaturesDefinitions(botName)

	for featureName, featureData := range featuresDefs {

		// Getting the type of the feature
		var featureType reflect.Type
		for fKey, fType := range knowFeatures {
			if fKey == featureName {
				featureType = fType
				break
			}
		...
		if featureType == nil {
			slog.Error(fmt.Sprintf("feature '%s' not registered, skipping", featureName))
			continue
		}

		// Create a new feature instance to unmarshal the data into
		featureInstance := reflect.New(featureType).Interface()

		// Retrieve the feature data from the configuration
		dataBytes, _ := toml.Marshal(featureData)

		// Unmarshal the data into the feature instance
		err := toml.Unmarshal(dataBytes, featureInstance)
		if err != nil {
			slog.Error(fmt.Sprintf("failed to unmarshal feature '%s': %s", featureName, err))
			continue
		}

		// Save the feature instance
		features = append(features, featureInstance.(Feature))
	}

	// // Iterating over the superset to create the features
	// for _, allowedFeature := range superset {

	// 	// Getting the type of the feature
	// 	featureType := reflect.TypeOf(allowedFeature)
	...
	// 	// Retrieve the feature data from the configuration
	// 	dataBytes, _ := toml.Marshal(featuresDefs[featureKey])
